...
def test_safe_softmax(M, N):
    logger.info(f"Testing safe softmax with M={M}, N={N}")
    # Create random input tensor
    input_tensor = torch.randn(M, N, device="cuda")

    # Compute softmax using the custom kernel
    output_custom = safe_softmax.safe_softmax(input_tensor)
    ...
    logger.info("Safe softmax test passed!")
